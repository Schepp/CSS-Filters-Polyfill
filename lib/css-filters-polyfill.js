				...
				switch(stylesheets[i].nodeName){
					...
					case 'LINK':
						if(stylesheets[i].rel === 'stylesheet'){
							...
							if(window.localStorage && window.localStorage.getItem('polyfilter_' + href)){
							...
							xmlHttp.onreadystatechange = function(){
								if(xmlHttp.readyState === 4){
									if(xmlHttp.status === 0){
										...
										if(window.localStorage) window.localStorage.setItem('polyfilter_' + href,polyfilter._stylesheets[index].content);
									}
								}
							};
							try{
