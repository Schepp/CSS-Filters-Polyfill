	...
	var polyfilter = {
		...
		_development_mode: (function(){
			if(location.hostname === 'localhost' || location.hostname.search(/.local$/) !== -1 || location.hostname.search(/\d+\.\d+\.\d+\.\d+/) !== -1){
				if(console) console.log('Detected localhost or IP address. Assuming you are a developer. Caching of stylesheets is disabled.');
				return true;
			}
			if(console) console.log('Caching of stylesheets is enabled. You need to refresh twice to see any changes.');
			return false;
		})(),
		...
		process_stylesheets: function(){
			...
			window.setTimeout(function(){
				...
				xmlHttp.open('GET', window.polyfilter_scriptpath + 'htc/sepia.htc', true);
							...
							try{
								...
								xmlHttp.onreadystatechange = function(){
									if(xmlHttp.readyState === 4){
										if(xmlHttp.status === 0){
											if(console) console.log('Could not fetch external CSS via HTTP-Request ' + href + '. Probably because of cross origin.');
											...
											if(!polyfilter._development_mode && window.localStorage){
												try{
													...
													if(console) console.log('Local storage quota (5MB) have been exceeded. Caching of stylesheet ' + href + ' is not possible');
												}
											}
										}
									}
								...
								try{
									...
									if(console) console.log('Could not fetch external CSS via HTTP-Request ' + href + '. Are you maybe testing using the file://-protocol?');
									if(!polyfilter._stylesheets[index].content){
										polyfilter._pending_stylesheets--;
										if(polyfilter._pending_stylesheets === 0){
											polyfilter.process();
