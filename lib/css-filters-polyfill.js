						...
						if(stylesheets[i].rel === 'stylesheet'){
							...
							xmlHttp.onreadystatechange = function(){
								if(xmlHttp.readyState === 4){
									...
									// Chache stylesheet in localStorage, if available
							...
							try{
								...
								if(console) console.log('Could not fetch external CSS via HTTP-Request. Are you maybe testing using the file://-protocol?');
								if(!polyfilter._stylesheets[index].content){
									polyfilter._pending_stylesheets--;
									if(polyfilter._pending_stylesheets === 0){
										polyfilter.process();
